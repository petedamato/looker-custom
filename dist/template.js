...
// EXTERNAL MODULE: ./node_modules/d3/index.js + 349 modules
var node_modules_d3 = __webpack_require__(0);
...
// CONCATENATED MODULE: ./src/integration/template/unquoted.js
...
var object = {
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    var convertNull = function convertNull(input) {
      if (input == null) {
        return 0;
      } else {
        return input;
    ...
    var convertNullString = function convertNullString(input) {
      if (input == null) {
        return "N/A";
      } else {
        return input;
    ...
    var checkForRule = function checkForRule(input, valObj) {
      var test = false;
      valObj.forEach(function (ent, i) {
        if (input == ent.rule) {
          test = i;
          return;
        }
    ...
    var palette = ["#27566b", "#8cbb61", "#007b82", "#f1cc56", "#339f7b"];
    var parseTimeDay = d3.timeParse("%Y-%m-%d");
    var parseTimeDayTime = d3.timeParse("%Y-%m-%d %H:%M:%S");
    var parseTime = d3.timeParse("%H:%M");
    var formatDate = d3.timeFormat("%b %d");
    d3.select("#vis").html(''); // set the dimensions and margins of the graph
    ...
    var pivots = queryResponse.fields.pivots; // Add an svg element for each group. The will be one beside each other and will go on the next row when no more room available

    var data_remix = [];
    var shipper_mix = [];
    var rules_order = [];
    var vals = Object.keys(data[0]["price_requests.price_requests_count"]);
    data.forEach(function (entry, i) {
      ...
      if (shipper_mix.includes(entry["shipper_applications.name"]["value"])) {
        data_remix.forEach(function (inter) {
          if (inter["shipper"] == entry["price_requests.shipper_application_id"]["value"]) {
            var checked = false;
            inter["rules"].forEach(function (new_entry) {
              if (new_entry["rule"] == entry["price_requests_spot_control_panel.id"]["value"]) {
                ...
                new_entry["values"].forEach(function (entry_value) {
    ...
    data_remix.forEach(function (entry, i) {
      entry.rules.sort(function (a, b) {
        return b["max"] - a["max"];
      });
      var data_container = [];
      vals.forEach(function (outer, i) {
        var pod = {};
        pod["total"] = 0;
        pod["week"] = outer;
        entry.rules.forEach(function (inner) {
    ...
    var shipperNest = d3.nest() // nest function allows to group the calculation per level of a factor
    .key(function (d) {
      return d.shipper_name;
    }).entries(data_remix);
    var finalData = shipperNest.filter(function (entry) {
      return entry["values"][0]["rules"][0]["max"] > 1;
    }); //END DATA MANIPULATION PHASE

    var localScale = d3.local();
    var localPath = d3.local();
    var localAxis = d3.local();
    var svg = d3.select("#vis").selectAll("uniqueChart").data(finalData).enter().append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("class", function (d, i) {
      return "group g" + "-" + i;
    }).attr("transform", "translate(" + margin.left + "," + margin.top + ")") // .attr("display", (d,i) => {
    //   if (d.values[0]["rules"][0]["max"] < 2) {
    //     return "none"
    //   }
    // })
    .each(function (d) {
      var yMax = d3.max(d.values[0].chart_values, function (num) {
        return num.total;
      });
      var yScale = localScale.set(this, d3.scaleLinear().domain([0, yMax]).range([height_chart, 0]));
      localPath.set(this, d3.area().curve(d3.curveMonotoneX).x(function (d) {
    ...
    svg.append("text").attr("text-anchor", "start").attr("y", -5).attr("x", 0).attr("font-size", 10).text(function (d) {
      return d.key;
    }); // Add X axis

    var x = d3.scaleTime().domain(d3.extent([parseTimeDay(vals[0]), parseTimeDay(vals[vals.length - 1])])).range([0, width]);
    var xAxis = svg.append("g").attr("class", "base-axis").attr("visibility", "visible").attr("transform", "translate(0," + height_chart + ")").call(d3.axisBottom(x).ticks(2));
    var xAxis2 = svg.append("g").attr("class", "scrollable-axis").attr("visibility", "hidden").attr("transform", "translate(0," + height_chart + ")").call(d3.axisBottom(x).ticks(d3.timeMonday.every(1)).tickFormat(function (d, i) {
      return formatDate(d);
    }));
    ...
    var y = d3.scaleLinear().domain([0, 10000]).range([height_chart, 0]); // Area generator
    ...
    d3.select("#vis").append("div").attr("class", "tooltip");
    var tooltip = d3.select(".tooltip").style("background-color", "#262d33").style("border-radius", "2px").style("padding", "5px").style("position", "absolute").style("pointer-events", "none").html("<div id='value-header'></div><div id='value-subheader'></div><div id='value-body'></div>"); // Add titles

    var rulesList = svg.selectAll(".rules-list").data(function (d) {
      return [d.values[0].rules];
    }).enter().append("g").attr("transform", "translate(0," + (height_chart + 20) + ")").append("foreignObject").attr("class", "rules-list-container");
    var rules = rulesList.append('xhtml:ul').attr('class', 'rules-list');
    rules.each(function (eac) {
      var ruleListContainer = d3.select(this); // $(this).append("li").attr("class", "rules-list")

      var rulesData = d3.select(this).data()[0];
      rulesData.forEach(function (entry, i) {
        ruleListContainer.append("li").attr("class", function (d, i) {
          if (entry.max > 1) {
          ...
          var strr = "<div class='rule-entry rect-" + entry.rule + "' style='background-color:" + palette[i % 5] + "'>";
          var reason = entry.reason.replace("Do not bid", "DNB");
          var labels = reason.split("|");
          ...
          var ruleBatch = entry.reason.split("|");
          var rules = '';
          var counter = 0;
          ruleBatch.forEach(function (entch) {
            if (entry == "N/A") {
              counter += 1;
            } else {
          ...
          tooltip.style("left", this.getBoundingClientRect().x + window.scrollX + "px").style("top", this.getBoundingClientRect().y + window.scrollY - 40 + "px");
    ...
    var mouseTracker = svg.append("rect").attr("fill", "none").attr("class", "mouse-tracker");
    var paths = svg.append("g").attr("class", "path-level").selectAll("mylayers").data(function (d, i) {
      var that_keys = d.values[0].rules.map(function (entry) {
        return String(entry.rule);
      }); //stacking

      var stackedData = d3.stack().keys(that_keys)(d.values[0].chart_values);
      return stackedData;
    }).enter(); // For later

    var chunk = width / vals.length;
    paths.append("path").attr("class", function (d, i) {
      ...
      d3.select(d3.event.target.parentNode).selectAll(".hoverLine").attr('opacity', 1).attr('x1', xCoord).attr('x2', xCoord); // d.key - path rule
      // d.index - sequential num

      $(event.target).parent().parent().children(".scrollable-axis").children().attr("opacity", 1);
      $(event.target).parent().parent().children(".scrollable-axis").children().not(".tick-" + Math.floor(xCoord / chunk)).attr("opacity", 0);
      $(".textg").children().attr("opacity", 0);
      $(".rect-" + d.key).children().not(".label-" + Math.floor(xCoord / chunk)).attr("opacity", 0);
      $(".rect-" + d.key + " > .label-" + Math.floor(xCoord / chunk)).attr("opacity", 1);
      var mouseDate = x.invert(xCoord); // const mouseDateSnap = d3.timeDay.floor(mouseDate);
      ...
      d3.selectAll('.hoverLine').attr('opacity', 0);
    });
    var textLayer = paths.append("g").attr("class", function (d, i) {
    ...
    textLayer.append("text").attr("class", function (d, i) {
      return "label-" + i;
    }).attr("x", function (d, i) {
      return x(parseTimeDay(d.data.week));
    }).attr("y", function (d, i) {
      var yMaxThis = $(this).parent().attr("class").split("yscale-")[1].split(" ")[0];

      if (d) {
        var yScaleThis = d3.scaleLinear().range([height_chart, 0]).domain([yMaxThis, 0]);
    ...
    }).attr("pointer-events", "none").attr("font-size", 12).text(function (d, i) {
      var yMaxThis = $(this).parent().attr("class").split("yscale-")[1].split(" ")[0];
    ...
    var mouseLine = d3.selectAll('.path-level').append('line').attr('class', 'hoverLine').attr("opacity", 0).attr("pointer-events", "none").attr('x1', 0).attr('x2', 0).attr('y1', 0).attr('y2', height_chart).attr('stroke', '#147F90').attr('fill', '#A6E8F2');
    shipperNest.forEach(function (ent, i) {
      var yMax = d3.max(ent.values[0].chart_values, function (d, i) {
        return d.total;
      });
      var grabber = ".g-" + i;
      y.domain([0, yMax]);
      ...
      $(grabber).children().children().children().children().children(".rule-entry").on("click", function (evt) {
        if ($(this).hasClass("clicked")) {
          ...
          var textClass = ".textg." + $(this).attr("class").split("rect-")[1];
          ...
          var _textClass = ".textg." + $(this).attr("class").split("rect-")[1];

          $(_textClass).addClass("seen");
          $(".rule-entry").removeClass("clicked");
          var indie = "." + $(this).attr("class").split(" ")[1];
          $(this).addClass("clicked");
          $(this).parent().parent().children().addClass("unclicked");
          $(".group" + grabber).children(".path-level").children(".myArea").removeClass("selected");
          $(".group" + grabber).children(".path-level").children(".myArea").attr("opacity", .1);
          $(".group" + grabber).children(".path-level").children(indie).addClass("selected"); // $(grabber).addClass("click-on")
        } else {
          var _textClass2 = ".textg." + $(this).attr("class").split("rect-")[1];

          $(_textClass2).addClass("seen");
          $(".group" + grabber + " .drop rect").removeClass("clicked");
          $(this).addClass("clicked");
          $(this).parent().parent().children().addClass("unclicked");
          $(".group" + grabber).children(".path-level").children(".myArea").attr("opacity", .1);

          var _indie = "." + $(this).attr("class").split(" ")[1];

          $(".group" + grabber).children(".path-level").children(_indie).addClass("selected");
...
keys.forEach(function (entry, i) {
  ...
  if (["radio", "select"].includes(object.options[entry].display)) {
    ...
    var _array_values2 = object.options[entry]["default"];
    console.log("array_values", _array_values2);
...
node_modules_d3["p" /* json */]("http://localhost:3001/dataUnquotedSpotFinal").then(function (data) {
  var todays_options = {};
  ...
  object.updateAsync(data.data, node_modules_d3["C" /* select */]("#viz")._groups[0][0], todays_options, data.queryResponse, details, done, this_environment); // On change to options, loop through selections and then redraw chart

  jquery('input:radio').on("click", function () {
    jquery('input:radio:checked').each(function () {
      todays_options[this.attributes.internal_cat.value] = this.attributes.internal_value.value;
    });
    object.updateAsync(data.data, node_modules_d3["C" /* select */]("#viz")._groups[0][0], todays_options, data.queryResponse, details, done, this_environment);
  });
