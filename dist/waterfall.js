...
looker.plugins.visualizations.add({
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    var colors = ["#27566b", "#8cbb61", "#007b82"];
    d3__WEBPACK_IMPORTED_MODULE_0__[/* select */ "C"]("#vis").select("svg").html(""); // TK
    ...
    var measuresIntermediate = queryResponse.fields.measure_like;
    var measures = [];
    measuresIntermediate.forEach(function (ent) {
    ...
    var x = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleLinear */ "x"]().domain([0, d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"]([d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"](data, function (entry) {
    ...
    var y = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleBand */ "w"]().domain(measures.map(function (ent) {
      return ent.name;
    })).range([height, 0]).padding(0.05);
    var pointsA = [{
      x: x.range()[0],
      y: y.bandwidth() + y(measures[2].name)
    }, {
      x: x.range()[1],
      y: y.bandwidth() + y(measures[2].name)
    }, {
      x: x.range()[1] - (width - x(data[0][measures[1].name]["value"])) / 2,
      y: y(measures[1].name) + 1
    }, {
      x: x.range()[0] + (width - x(data[0][measures[1].name]["value"])) / 2,
      y: y(measures[1].name) + 1
    }]; // const pointsA = [{
    //     x: x.range()[0], y: 100
    //   },{
    ...
    var pointsB = [{
      x: x.range()[0] + (width - x(data[0][measures[1].name]["value"])) / 2,
      y: (y.bandwidth() + y(measures[2].name)) * 2
    }, {
      x: x.range()[1] - (width - x(data[0][measures[1].name]["value"])) / 2,
      y: (y.bandwidth() + y(measures[2].name)) * 2
    }, {
      x: x.range()[1] - (width - x(data[0][measures[1].name]["value"]) * 0.1) / 2,
      y: y(measures[0].name)
    }, {
      x: x.range()[0] + (width - x(data[0][measures[1].name]["value"]) * 0.1) / 2,
      y: y(measures[0].name)
    }];
    var line = d3__WEBPACK_IMPORTED_MODULE_0__[/* line */ "q"]().x(function (d) {
      return d.x;
    }).y(function (d) {
      return d.y;
    });
    svg.append('path').attr("d", line(pointsA) + 'Z').style("fill", "black").style("stroke", "black").attr("opacity", .1);
    svg.append('path').attr("d", line(pointsB) + 'Z').style("fill", "black").style("stroke", "black").attr("opacity", .1); // append the rectangles for the bar chart

    svg.selectAll(".bar-thresh").data(measures).enter().append("rect").attr("class", "bar-thresh").attr("x", function (d) {
      ...
      return y.bandwidth();
    }).attr("fill", "black").attr("opacity", .1); // append the rectangles for the bar chart

    svg.selectAll(".bar").data(measures).enter().append("rect").attr("class", "bar").attr("x", function (d) {
    ...
    var threshVals = [measures[0], measures[1], measures[1]];
    svg.selectAll(".bar-thresh-label-rects").data(threshVals).enter().append("rect").attr("class", "bar-thresh-label-rects").attr("x", function (d, i) {
      return width - [150, 100][i] - 20;
    }).attr("y", function (d, i) {
      return y("price_requests.won_shipment_count") + y.bandwidth() / 2 - 35 + i * 30;
    }).attr("width", function (d, i) {
      return [150, 100][i];
    }).attr("height", 26).attr("opacity", function (d, i) {
      if (i == 0) {
        return 1;
      } else if (i == 1) {
        return .1;
    ...
    svg.selectAll(".bar-thresh-label").data(threshVals).enter().append("text").attr("class", "bar-thresh-label").attr("x", function (d, i) {
      return width - 26;
    }).attr("y", function (d, i) {
      if (i != 2) {
      ...
      if (i == 0) {
        return ["Shipments Won", "Target*"][i] + ": " + d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"](',')(data[0][d.name]["value"]);
      } else if (i == 1) {
        return ["Shipments Won", "Target"][i] + ": " + d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"](',')(Math.round(data[0][d.name]["value"] / 10));
      } else {
    ...
    svg.selectAll(".bar-label").data(measures).enter().append("text").attr("class", "bar-label").attr("x", function (d, i) {
      if (i == 0) {
        return width / 2 + 12;
      } else {
        return width / 2;
      ...
      return ["Shipments Won", "Shipments Quoted", "Shipments Available"][i] + ": " + d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"](',')(data[0][d.name]["value"]);
