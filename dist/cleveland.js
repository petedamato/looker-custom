(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["cleveland"] = factory();
	else
		root["cleveland"] = factory();
})(window, function() {
...
looker.plugins.visualizations.add({
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    var measures = queryResponse.fields.measure_like; // Clear any errors from previous updates

    this.clearErrors();
    var svg = this.svg.html('').attr('width', '100%').attr('height', '100%').append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')'); // TODO Add option to sort on gap between measures
    // data.sort(function(a,b){
    //    return (a[measures[0].name]["value"] - a[measures[1].name]["value"]) - (b[measures[0].name]["value"] - b[measures[1].name]["value"]);
    //  })

    var x = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleBand */ "w"]().domain(data.map(function (entry) {
    ...
    })]), d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"]([d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"](data, function (entry) {
      return entry[measures[0].name]["value"];
    }), d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"](data, function (entry) {
      return entry[measures[1].name]["value"];
    })])]).range([height, 0]);
    var line1 = d3__WEBPACK_IMPORTED_MODULE_0__[/* area */ "a"]().curve(d3__WEBPACK_IMPORTED_MODULE_0__[/* curveBasis */ "e"]).x(function (d) {
      return x(d[dimension.name]["value"]);
    }).y(function (d) {
      return y(d[measures[0].name]["value"]);
    });
    var line2 = d3__WEBPACK_IMPORTED_MODULE_0__[/* area */ "a"]().curve(d3__WEBPACK_IMPORTED_MODULE_0__[/* curveBasis */ "e"]).x(function (d) {
      return x(d[dimension.name]["value"]);
    }).y(function (d) {
      return y(d[measures[1].name]["value"]);
    });
    var area = d3__WEBPACK_IMPORTED_MODULE_0__[/* area */ "a"]().curve(d3__WEBPACK_IMPORTED_MODULE_0__[/* curveBasis */ "e"]).x(function (d) {
      return x(d[dimension.name]["value"]);
    }).y1(function (d) {
      return y(d[measures[0].name]["value"]);
    });
    var xAxis = d3__WEBPACK_IMPORTED_MODULE_0__[/* axisBottom */ "b"](x).tickSize(0);
    var yAxis = d3__WEBPACK_IMPORTED_MODULE_0__[/* axisLeft */ "c"](y).tickSize(0).ticks(5).tickFormat(d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"]("$,d"));
    var yGridlines = d3__WEBPACK_IMPORTED_MODULE_0__[/* axisLeft */ "c"](y).tickSize(-width, 0, 0).tickFormat("");

    if (config.chart_type == "dot") {
      // gridlines
      svg.append("g").call(yGridlines).classed("gridline", true).attr("transform", "translate(0,0)");
      svg.selectAll(".gap").data(data).enter().append("line").classed("gap", true).style("stroke", "#91a1a8").attr("x1", function (d, i) {
      ...
      svg.selectAll(".dot").data(data).enter().append("circle").classed("dot", true).attr("r", 8).attr("cy", function (d, i) {
        return y(d[measures[0].name]["value"]);
      }).attr("cx", function (d, i) {
        return x(d[dimension.name]["value"]) + x.bandwidth() / 2;
      }).attr("fill", colors[0]);
      svg.selectAll(".lowdot").data(data).enter().append("circle").classed("lowdot", true).attr("cy", function (d, i) {
        return y(d[measures[1].name]["value"]);
      }).attr("cx", function (d, i) {
        return x(d[dimension.name]["value"]) + x.bandwidth() / 2;
      }).attr("r", 8).attr("fill", colors[1]);
      ...
      svg.append("path").attr("class", "area above").attr("clip-path", "url(#clip-above)").attr("d", area.y0(function (d) {
        return y(d[measures[1].name]["value"]);
      })).attr("transform", "translate(" + x.bandwidth() / 2 + ",0)").attr("fill", fills[1]);
      svg.append("path").attr("class", "area below").attr("clip-path", "url(#clip-below)").attr("d", area).attr("transform", "translate(" + x.bandwidth() / 2 + ",0)").attr("fill", fills[0]);
      svg.append("path").attr("class", "line1").attr("d", line1).attr("transform", "translate(" + x.bandwidth() / 2 + ",0)").attr("fill", "none").attr("stroke", colors[0]).attr("stroke-width", 3);
      svg.append("path").attr("class", "line2").attr("d", line2).attr("transform", "translate(" + x.bandwidth() / 2 + ",0)").attr("fill", "none").attr("stroke", colors[1]).attr("stroke-width", 3);
    } // axes


    svg.append("g").classed("x axis", true).attr("transform", "translate(" + 0 + "," + (height - 1) + ")").call(xAxis).selectAll("text").style("text-anchor", "end").attr("dx", -4).attr("dy", 10).attr("transform", "rotate(-40)").style("font-size", 10);
    ...
    var legend = svg.selectAll(".legend").data(colors).enter().append("g").attr("class", "legend").attr("transform", function (d, i) {
      return "translate(" + (i * 180 - width / 2.05) + "," + (height + 50) + ")";
    });
    legend.append("circle").attr("cx", width - 12).attr("cy", 9).attr("r", 7).style("fill", function (d) {
      return d;
    });
