(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["groupedStackedAB"] = factory();
	else
		root["groupedStackedAB"] = factory();
})(window, function() {
...
looker.plugins.visualizations.add({
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    var dataColumns = Object.keys(data[0]).splice(1);
    var pivotedColumns = Object.keys(data[0][dataColumns[0]]);
    var dat_data = data;
    var manip_data = [];
    dat_data.forEach(function (d) {
      ...
      for (var i = 0; i < pivotedColumns.length; i++) {
        var inner_group = {};
    ...
    var parseDate = d3__WEBPACK_IMPORTED_MODULE_0__[/* timeParse */ "I"]("%Y-%m-%d");
    var formatDate = d3__WEBPACK_IMPORTED_MODULE_0__[/* timeFormat */ "H"]("%b %d, '%y");
    var heightA = height * (1 / 3),
        heightB = height / 3;
    var groupA = svg.append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('width', "100%").attr('height', heightA - margin.top - margin.bottom + "px").classed("groupA", true);
    var groupB = svg.append('g').attr('width', "100%").attr('height', heightB - margin.top - margin.bottom + "px").attr('transform', 'translate(' + margin.left + ',' + (heightA + margin.top + margin.top) + ')').classed("groupB", true);
    var usable_data = manip_data;
    var dimensions = [];

    var stripedPattern = function () {
      for (var i = 0, ii = usable_data.length; i < ii; i++) {
        //implement checker: check for real month
        if (usable_data[i].period === '2022-05-16') {
          return {
            periodName: '2022-05-16'
          };
        }
      }
    }(); //d3 scale stuff
    ...
    var x0 = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleBand */ "w"]().rangeRound([0, width]).padding(0.1);
    var x1 = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleBand */ "w"]();
    var y = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleLinear */ "x"]().range([heightA, 0]);
    var yBegin; //fetch the column headers

    var itemLookup = usable_data[0];
    var groups = Object.keys(itemLookup.group);
    var segments = Object.keys(itemLookup.group[groups[0]]);
    var columnHeaders = [];

    var innerColumns = function () {
      var result = {};
    ...
    var dataRebuild = []; // Determine the shape of the dataset in order to match colors to it.
    ...
    usable_data.forEach(function (d, i) {
      ...
      if (d.period === stripedPattern.periodName) {// chart
        //   .select('svg')
        //   .append('defs');
      }
    ...
    dataRebuild.forEach(function (d) {
      ...
      d.columnDetails = columnHeaders.map(function (name) {
        for (var ic in innerColumns) {
          if (jquery__WEBPACK_IMPORTED_MODULE_1__["inArray"](name, innerColumns[ic]) >= 0) {
            if (!yColumn[ic]) {
              yColumn[ic] = 0;
            }
      ...
      d.total = d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"](d.columnDetails, function (d) {
        return d.yEnd;
      });
    }); //get month names

    x0.domain(dataRebuild.map(function (d) {
      return parseDate(d.periodName);
    })); //get something

    x1.domain(Object.keys(innerColumns)).rangeRound([0, x0.bandwidth()], .01, 0); //set y domain, get totals

    y.domain([0, d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"](dataRebuild, function (d) {
      return d.total;
    })]);
    var stackedBars = groupA.selectAll(".stackedBars").data(dataRebuild).enter().append('g').attr("class", function (d) {
      return 'month-col ' + parseDate(d.periodName);
    }).attr("transform", function (d) {
      return "translate(" + x0(parseDate(d.periodName)) + ",0)";
    });
    var bars = stackedBars.selectAll("rect").data(function (d) {
      return d.columnDetails;
    });
    var colorMaker = ["rgba(39, 86, 107,", "rgba(140, 187, 97,", "rgba(0, 123, 130,", "rgba(241,205,86,"];
    var colors = [];
    ...
    bars.enter().append("rect").attr("class", "item").attr("x", function (d) {
      return x1(d.group) + 1;
    ...
    bars.enter().append("text").attr("class", "item-text").attr("x", function (d) {
    ...
    groupA.selectAll(".month-col").data(dataRebuild).append('g').attr("class", 'legend-item').attr("text-anchor", "middle").attr("transform", function (d) {
      ...
      return formatDate(parseDate(d.periodName));
    }); //totals

    groupA.selectAll(".month-col").data(dataRebuild).append('g').attr("class", "test");
    groupA.append("g").attr("transform", "translate(" + -4 + ",0)").call(d3__WEBPACK_IMPORTED_MODULE_0__[/* axisLeft */ "c"](y).tickFormat(function (d) {
      return d;
    }).tickSize(0).ticks(4));
    groupA.append("text").attr("transform", "translate(" + -24 + ",-4)").attr("font-weight", "bold").text("Predicted appointment - successfully scheduled | not scheduled");
    var legend = groupA.selectAll(".legend").data(groups).enter().append("g").attr("class", "legend").attr("transform", function (d, i) {
      return "translate(" + (-40 + i * 110 - width / 2.05) + "," + -margin.top / 2 + ")";
    });
    legend.append("circle").attr("cx", width - 12).attr("cy", 9).attr("r", 7).style("fill", function (d, i) {
      return colors[i * 2];
    });
    legend.append("text").attr("x", width - 24).attr("y", 9).attr("dy", ".35em").style("text-anchor", "end").style("font-size", 10).text(function (d, i) {
      return d;
    });
    var line = d3__WEBPACK_IMPORTED_MODULE_0__[/* line */ "q"]().x(function (d, i) {
      return x0(parseDate(d.periodName));
    }).y(function (d, i) {
      var denominator = dataColumns[1] + "-" + pivotedColumns[0];
      var numerator = dataColumns[0] + "-" + pivotedColumns[0];
      return yB(d[numerator] / (d[denominator] + d[numerator]));
    });
    var line2 = d3__WEBPACK_IMPORTED_MODULE_0__[/* line */ "q"]().x(function (d, i) {
      return x0(parseDate(d.periodName));
    }).y(function (d, i) {
      var denominator = dataColumns[1] + "-" + pivotedColumns[1];
      var numerator = dataColumns[0] + "-" + pivotedColumns[1];
      return yB(d[numerator] / (d[denominator] + d[numerator]));
    });
    var marginB = {
      top: 35,
      ...
      left: 40
        ...
        yB = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleLinear */ "x"]().range([heightB, 0]).domain([0, 1]);
    groupB.append("g").attr("transform", "translate(" + -4 + ",0)").call(d3__WEBPACK_IMPORTED_MODULE_0__[/* axisLeft */ "c"](yB).tickFormat(function (d) {
      return d * 100 + "%";
    }).tickSize(0).ticks(4));
    groupB.append("text").attr("transform", "translate(" + -30 + ",-12)").attr("text-anchor", "beginning").attr("font-weight", "bold").text("Conversion Rate");
    var path = groupB.append("path").data([dataRebuild]).attr("class", "line").attr("d", line).attr('transform', 'translate(' + x0.bandwidth() / 2 + ',' + 0 + ')').attr("fill", "rgba(39, 86, 107,1)");
    var dot = groupB.selectAll(".dot").data(dataRebuild).enter().append('g').attr('transform', 'translate(' + x0.bandwidth() / 2 + ',' + 0 + ')').attr("class", "dot");
    ...
    }).attr("text-anchor", "middle").attr("fill", "white").attr("font-weight", "bold").text(function (d) {
      var denominator = dataColumns[1] + "-" + pivotedColumns[0];
      var numerator = dataColumns[0] + "-" + pivotedColumns[0];
    ...
    dot.append("text").attr("x", function (d) {
      ...
      var numerator = dataColumns[0] + "-" + pivotedColumns[0];
      return yB(d[numerator] / (d[denominator] + d[numerator])) - 8;
    }).attr("text-anchor", "middle").attr("fill", "rgba(39, 86, 107,1)").text(function (d) {
      var denominator = dataColumns[1] + "-" + pivotedColumns[0];
      var numerator = dataColumns[0] + "-" + pivotedColumns[0];
      return String(Math.round(d[numerator] * 100 / (d[denominator] + d[numerator]), 0)) + "%";
    });
    var path2 = groupB.append("path").data([dataRebuild]).attr("class", "line2").attr("d", line2).attr('transform', 'translate(' + x0.bandwidth() / 2 + ',' + 0 + ')').attr("fill", "black");
    var dot2 = groupB.selectAll(".dotB").data(dataRebuild).enter().append('g').attr('transform', 'translate(' + x0.bandwidth() / 2 + ',' + 0 + ')').attr("class", "dotB");
    dot2.append("circle").classed('dot-boi', true).attr("r", 6).attr("cx", function (d) {
      // This places the circles on the chart, using scaled x vals
      return x0(parseDate(d.periodName));
    }).attr("cy", function (d) {
      var denominator = dataColumns[1] + "-" + pivotedColumns[1];
    ...
    dot2.append("circle").classed('dot-pupil', true).attr("r", 3).attr("cx", function (d) {
      // This places the circles on the chart, using scaled x vals
      return x0(parseDate(d.periodName));
    }).attr("cy", function (d) {
      var denominator = dataColumns[1] + "-" + pivotedColumns[1];
    ...
    dot2.append("text").classed("drop-shadow-text", true).attr("x", function (d) {
      // This places the circles on the chart, using scaled x vals
      return x0(parseDate(d.periodName));
    }).attr("y", function (d) {
      var denominator = dataColumns[1] + "-" + pivotedColumns[1];
    ...
    dot2.append("text").attr("x", function (d) {
      // This places the circles on the chart, using scaled x vals
      return x0(parseDate(d.periodName));
    }).attr("y", function (d) {
      var denominator = dataColumns[1] + "-" + pivotedColumns[1];
      var numerator = dataColumns[0] + "-" + pivotedColumns[1];
      return yB(d[numerator] / (d[denominator] + d[numerator])) + 18;
    }).attr("text-anchor", "middle").attr("fill", "rgba(140, 187, 97,1)").text(function (d) {
      var denominator = dataColumns[1] + "-" + pivotedColumns[1];
      var numerator = dataColumns[0] + "-" + pivotedColumns[1];
      return String(Math.round(d[numerator] * 100 / (d[denominator] + d[numerator]), 0)) + "%";
    ...
    if (queryResponse.fields.dimensions.length == 0) {
      ...
      this.addError({
        title: "No Chart Type",
        message: "Select either Area or Dot from the options."
      });
      return;
    } else if (queryResponse.fields.measures.length < 2 || queryResponse.fields.measures.length > 2) {
      this.addError({
        title: "Measure Error",
        message: "This chart type can only display two measures at a time."
      });
      return;
