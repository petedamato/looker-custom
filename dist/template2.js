(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["template2"] = factory();
	else
		root["template2"] = factory();
})(window, function() {
...
// CONCATENATED MODULE: ./src/integration/template/treemap.js
var object = {
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    var pivots = Object.keys(data[0]["price_requests.quoted_shipment_count"]);
    var data_remix_rule = [];
    ...
    var y = d3.scaleLinear().domain([0, height - margin.top - margin.bottom]).range([0, height - margin.top - margin.bottom]);
    var color = d3.scaleOrdinal().range(["#27566b", "#8cbb61", "#007b82", "#f1cc56", "#339f7b"].map(function (c) {
      c = d3.rgb(c);
      c.opacity = 0.95;
      return c;
    ...
    function updateDrillDown() {
      if (svg) {
        ...
        //        .round(false);
        svg = d3.select("#vis").select("svg").attr("width", width - margin.left - margin.right).attr("height", height - margin.bottom - margin.top).style("margin-left", -margin.left + "px").style("margin.right", -margin.right + "px").append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")").style("shape-rendering", "crispEdges");
        ...
        treemap = d3.treemap().tile(d3.treemapResquarify.ratio(height / width * 0.5 * (1 + Math.sqrt(5)))).size([width, height]).round(false).paddingInner(1);
      }

      var root = d3.hierarchy(data2).eachBefore(function (d) {
    ...
    function display(d) {
      ...
      function transition(d) {
        ...
        t1.remove().on("end", function () {
          ...
          transitioning = false;
          d3.selectAll(".parent").on("mouseover", function () {
    ...
    var tooltip = d3.select(".tooltip").style("background-color", "#262d33").style("border-radius", "2px").style("padding", "5px").style("position", "absolute").style("pointer-events", "none").html("Shipper");
    d3.selectAll(".parent").on("mouseover", function () {
      tooltip.attr("class", "tooltip visible");
      tooltip.html($(this).attr("label"));
      tooltip.style("left", d3.mouse(d3.event.target)[0] + "px").style("top", d3.mouse(d3.event.target)[1] + "px").style("opacity", 1);
...
// CONCATENATED MODULE: ./src/integration/template/template2.js
...
node_modules_d3["p" /* json */]("http://localhost:3001/dataUnquotedShipments").then(function (data) {
  var todays_options = {};
