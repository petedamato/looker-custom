(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["spotStacked"] = factory();
	else
		root["spotStacked"] = factory();
})(window, function() {
...
looker.plugins.visualizations.add({
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    var pivotKeys = Object.keys(data[0][measures[0].name]);
    var pivotKey = pivotKeys[pivotKeys.length - 1];
    var listPeg = jquery__WEBPACK_IMPORTED_MODULE_1__('#vis-options');
    listPeg.empty().append(jquery__WEBPACK_IMPORTED_MODULE_1__('<option></option>').attr('value', pivotKeys[pivotKeys.length - 1]).text("All").attr("selected", "selected"));
    pivotKeys.forEach(function (entry) {
      if (entry.includes("null") || entry.includes("$")) {
        return;
      } else {
        listPeg.append(jquery__WEBPACK_IMPORTED_MODULE_1__('<option></option>').attr('value', entry).text(entry.split("|")[0]));
      }
    });
    ...
    var pivotedColumns = Object.keys(data[0]);
    var dataColumns = ["asp_quotes", "price_requests.quoted_shipment_count", "price_requests.available_shipment_count"];
    pivotedColumns = pivotedColumns.filter(function (entry) {
      return entry != dimension.name;
    });

    function redraw() {
      pivotKey = jquery__WEBPACK_IMPORTED_MODULE_1__('#vis-options option:selected').val();
      d3__WEBPACK_IMPORTED_MODULE_0__[/* select */ "C"]("#vis").select("svg").html(""); // append the svg object to the body of the page

      var svg = d3__WEBPACK_IMPORTED_MODULE_0__[/* select */ "C"]("#vis").select("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom);
      var parseDate = d3__WEBPACK_IMPORTED_MODULE_0__[/* timeParse */ "I"]("%Y-%m");
      var formatDate = d3__WEBPACK_IMPORTED_MODULE_0__[/* timeFormat */ "H"]("%b %d, '%y"); // //holder for the data obj rebuild

      var dataRebuild = [];
      var dataRebuildLine = []; // // Determine the shape of the dataset in order to match colors to it.
      ...
      var stackedData = d3__WEBPACK_IMPORTED_MODULE_0__[/* stack */ "E"]().keys(dataColumns)(dataRebuild);
      var heightA = height * (3 / 4),
          heightB = height / 4 - 30;
      var groupA = svg.append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('width', "100%").attr('height', heightA + "px").classed("groupA", true);
      svg.append('rect').attr('width', width + margin.left / 2 + margin.right / 2).attr('height', heightB + 50 + "px").attr('transform', 'translate(' + 10 + ',' + (heightA + 12) + ')').classed("backdrop", true).style("fill", "#e9eaf3");
      var groupB = svg.append('g').attr('width', "100%").attr('height', heightB - margin.top - margin.bottom + "px").attr('transform', 'translate(' + margin.left + ',' + (heightA + margin.top / 1.8 + 16) + ')').classed("groupB", true).style("background-color", "black"); //d3 scale stuff

      var x = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleBand */ "w"]().rangeRound([0, width]).padding(0.1); // const x1 = d3.scaleBand()

      var y1 = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleLinear */ "x"]().domain([0, d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"](dataRebuild, function (d) {
        return d["total"];
      })]).range([heightA - margin.top - margin.bottom, 0]); // const y2 = d3.scaleLinear().range([heightB, 0])
      // //add Domain here
      ...
      groupA.selectAll(".month-col").data(dataRebuild).append('g').attr("class", 'legend-item').attr("text-anchor", "middle").attr("transform", function (d) {
        return "translate(" + svg.select('.item').attr('width') + ',' + parseInt(heightA + 16) + ")";
      }).append('text').text(function (d) {
        return d.group;
      }); //totals

      groupA.selectAll(".month-col").data(dataRebuild).append('g').attr("class", "test");
      groupA.append("g").attr("class", "yAxis").attr("transform", "translate(" + -4 + ",0)").call(d3__WEBPACK_IMPORTED_MODULE_0__[/* axisLeft */ "c"](y1).tickFormat(function (d) {
        return d;
      }).tickSize(0).ticks(4));
      groupA.append("g").attr("class", "xAxis").attr("transform", "translate(" + 0 + "," + (heightA - margin.bottom - margin.top) + ")").call(d3__WEBPACK_IMPORTED_MODULE_0__[/* axisBottom */ "b"](x).tickFormat(function (d) {
        return d;
      }).tickSize(0));
      groupA.selectAll("g.yAxis g.tick").append("line").attr("class", "gridline").attr("x1", 0).attr("y1", 0).attr("x2", width).attr("y2", 0);
      var color = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleOrdinal */ "y"]().domain(dataColumns).range(['#27566b', '#007b82', '#969FA0']);
      d3__WEBPACK_IMPORTED_MODULE_0__[/* select */ "C"]("#vis").append("div").attr("class", "tooltip");
      var tooltip = d3__WEBPACK_IMPORTED_MODULE_0__[/* select */ "C"](".tooltip").attr("opacity", 0).style("display", "none").style("background-color", "#262d33").style("border-radius", "2px").style("padding", "5px").style("position", "absolute").style("pointer-events", "none").html("<div id='value-body'></div>");
      ...
      var mousemove = function mousemove(d) {
        tooltip.style("left", d3__WEBPACK_IMPORTED_MODULE_0__[/* mouse */ "t"](this)[0] + "px").style("top", d3__WEBPACK_IMPORTED_MODULE_0__[/* mouse */ "t"](this)[1] - 25 + "px"); // tooltipDimensionBody.html("<span>" + d.data.group + "</span>")
      ...
      var groupABars = groupA.selectAll(".bar-groups") // Enter in the stack data = loop key per key = group per group
      .data(stackedData).enter().append("g").attr("class", "bar-groups").attr("fill", function (d) {
        return color(d.key);
      });
      groupABars.selectAll("rect") // enter a second time = loop subgroup per subgroup to add all rectangles
      .data(function (d) {
        return d;
      ...
      }).attr("width", x.bandwidth()).on("mouseover", mouseover).on("mousemove", mousemove).on("mouseout", mouseleave);
      groupA.selectAll(".bar-groups-text") // Enter in the stack data = loop key per key = group per group
      .data(stackedData).enter().append("g").attr("class", "bar-groups-text").attr("pointer-events", "none").selectAll("text") // enter a second time = loop subgroup per subgroup to add all rectangles
      .data(function (d) {
        return d;
      }).enter().append("text").attr("x", function (d) {
        return x(d.data.group) + x.bandwidth() / 2;
      }).attr("y", function (d) {
        return y1(d[0]) - 2;
      }).attr("text-anchor", "middle").attr("font-size", 12).attr("display", function (d) {
      ...
      d3__WEBPACK_IMPORTED_MODULE_0__[/* selectAll */ "D"](".tick text").attr("font-family", "Roboto"); // groupA.append("text")
      //   .attr("transform", "translate(" + -24 + ",-4)")
      //   .attr("font-weight", "bold")
      //   .text("Predicted appointment - successfully scheduled | not scheduled")

      var legend = groupA.selectAll(".legend").data(dataColumns).enter().append("g").attr("class", "legend").attr("transform", function (d, i) {
        return "translate(" + i * width / 2.8 + "," + -margin.top / 1.5 + ")";
      });
      legend.append("circle").attr("cx", 0).attr("cy", 9).attr("r", 7).style("fill", function (d, i) {
        return color.range()[i];
      });
      legend.append("text").attr("x", 10).attr("y", 9).attr("dy", ".35em").style("text-anchor", "start").style("font-size", 10).text(function (d, i) {
        var dat = ["Quoted shipments, ASP", "Quoted shipments, other", "Unquoted shipments"];
        return dat[i];
      }); // const line2 = d3.line()
      //     .x(function(d, i) {
      ...
      var y2max = d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"]([d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"](dataRebuildLine, function (d) {
        return d.wr;
      }) + 0.05, 0.1]);
      var y2 = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleLinear */ "x"]().range([heightB, 0]).domain([0, y2max]);
      groupB.append("g").attr("class", "yAxis").attr("transform", "translate(" + -4 + ",0)").call(d3__WEBPACK_IMPORTED_MODULE_0__[/* axisLeft */ "c"](y2).tickFormat(function (d) {
        return d * 100 + "%";
      }).tickSize(0).ticks(2));
      groupB.selectAll("g.yAxis g.tick").append("line").attr("class", "gridline2").attr("x1", 0).attr("y1", 0).attr("x2", width).attr("y2", 0); // groupB.append("text")
      //   .attr("transform", "translate(" + (-30) + ",-12)")
      //   .attr("text-anchor", "beginning")
      //   .attr("font-weight", "bold")
      //   .text("Conversion Rate")

      var path = groupB.append("path").datum(data).attr('transform', 'translate(' + x.bandwidth() / 2 + ',' + 0 + ')').attr("fill", "none").attr("stroke", "rgba(39, 86, 107,1)").attr("stroke-width", 1.5).attr("d", d3__WEBPACK_IMPORTED_MODULE_0__[/* line */ "q"]().x(function (d) {
        return x(d["price_requests.created_month"].value);
      }).y(function (d) {
        return y2(d["price_requests.win_percent"][pivotKey].value);
      }));
      groupB.append("text").attr("x", (width - margin.left - margin.right) / 1.9).attr("text-anchor", "middle").attr("y", -6).attr("font-family", "Roboto").style("font-size", 12).text("Win rate");
      var dot = groupB.selectAll(".dot").data(data).enter().append('g').attr('transform', 'translate(' + x.bandwidth() / 2 + ',' + 0 + ')').attr("class", "dot");
      dot.append("circle").classed('dot-boi', true).attr("r", 6).attr("cx", function (d) {
        // This places the circles on the chart, using scaled x vals
        return x(d["price_requests.created_month"].value);
      }).attr("cy", function (d) {
        return y2(d["price_requests.win_percent"][pivotKey].value);
      }).attr("fill", "rgba(39, 86, 107,1)");
      dot.append("circle").classed('dot-pupil', true).attr("r", 3).attr("cx", function (d) {
        // This places the circles on the chart, using scaled x vals
        return x(d["price_requests.created_month"].value);
      }).attr("cy", function (d) {
        return y2(d["price_requests.win_percent"][pivotKey].value);
      }).attr("fill", "#e9eef0");
      dot.append("text").classed("drop-shadow-text", true).attr("x", function (d) {
        // This places the circles on the chart, using scaled x vals
        return x(d["price_requests.created_month"].value);
      }).attr("y", function (d) {
        return y2(d["price_requests.win_percent"][pivotKey].value) - 8;
      }).attr("text-anchor", "middle").attr("fill", "white").attr("font-size", 12).attr("font-weight", "bold").text(function (d) {
        return Math.round(d["price_requests.win_percent"][pivotKey].value * 1000) / 10 + "%";
      });
      dot.append("text").attr("x", function (d) {
        // This places the circles on the chart, using scaled x vals
        return x(d["price_requests.created_month"].value);
      }).attr("y", function (d) {
        return y2(d["price_requests.win_percent"][pivotKey].value) - 8;
      }).attr("text-anchor", "middle").attr("font-size", 12).attr("fill", "rgba(39, 86, 107,1)").text(function (d) {
        return Math.round(d["price_requests.win_percent"][pivotKey].value * 1000) / 10 + "%";
      });
    }

    redraw(); // Callback at the end of the rendering to let Looker know it's finished
