(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["difference"] = factory();
	else
		root["difference"] = factory();
})(window, function() {
...
/* harmony import */ var _common_utils__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(9);
...
looker.plugins.visualizations.add({
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    if (environment == "prod") {
      if (!Object(_common_utils__WEBPACK_IMPORTED_MODULE_1__[/* handleErrors */ "a"])(this, queryResponse, {
        min_pivots: 0,
        max_pivots: 0,
        min_dimensions: 1,
    ...
    var colors;
    var fills = ['#d4dde2', '#d4dde2'];
    colors = ['rgb(39, 86, 107)', 'rgb(140, 187, 97)']; // TK
    ...
    var svg = d3__WEBPACK_IMPORTED_MODULE_0__[/* select */ "C"]("#vis").select("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); // // Clear any errors from previous updates
    // this.clearErrors();
    // const svg = (
    //   this.svg
    //   .html('')
    ...
    var parseTime = d3__WEBPACK_IMPORTED_MODULE_0__[/* timeParse */ "I"]("%Y-%m");
    var x = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleTime */ "B"]().domain(d3__WEBPACK_IMPORTED_MODULE_0__[/* extent */ "k"](data, function (entry) {
      return parseTime(entry[dimension.name]["value"]);
    })).range([0, width]);
    var y = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleLinear */ "x"]().domain([d3__WEBPACK_IMPORTED_MODULE_0__[/* min */ "s"]([d3__WEBPACK_IMPORTED_MODULE_0__[/* min */ "s"](data, function (entry) {
      return entry[measures[0].name]["value"];
    }), d3__WEBPACK_IMPORTED_MODULE_0__[/* min */ "s"](data, function (entry) {
      return entry[measures[1].name]["value"];
    })]) * .8, d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"]([d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"](data, function (entry) {
      return entry[measures[0].name]["value"];
    }), d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"](data, function (entry) {
      return entry[measures[1].name]["value"];
    })]) * 1.2]).range([height, 0]);
    var bisectDate = d3__WEBPACK_IMPORTED_MODULE_0__[/* bisector */ "d"](function (d) {
      return parseTime(d[dimension.name]["value"]);
    }).left;
    var line1 = d3__WEBPACK_IMPORTED_MODULE_0__[/* area */ "a"]().curve(d3__WEBPACK_IMPORTED_MODULE_0__[/* curveLinear */ "g"]).x(function (d) {
      return x(parseTime(d[dimension.name]["value"]));
    }).y(function (d) {
      return y(d[measures[0].name]["value"]);
    });
    var line2 = d3__WEBPACK_IMPORTED_MODULE_0__[/* area */ "a"]().curve(d3__WEBPACK_IMPORTED_MODULE_0__[/* curveLinear */ "g"]).x(function (d) {
      return x(parseTime(d[dimension.name]["value"]));
    }).y(function (d) {
      return y(d[measures[1].name]["value"]);
    });
    var area = d3__WEBPACK_IMPORTED_MODULE_0__[/* area */ "a"]().curve(d3__WEBPACK_IMPORTED_MODULE_0__[/* curveLinear */ "g"]).x(function (d) {
      return x(parseTime(d[dimension.name]["value"]));
    }).y1(function (d) {
      return y(d[measures[0].name]["value"]);
    });
    var xAxis = d3__WEBPACK_IMPORTED_MODULE_0__[/* axisBottom */ "b"](x).tickSize(0);
    var yAxis = d3__WEBPACK_IMPORTED_MODULE_0__[/* axisLeft */ "c"](y).tickSize(0).ticks(5).tickFormat(d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"](".0%"));
    var yGridlines = d3__WEBPACK_IMPORTED_MODULE_0__[/* axisLeft */ "c"](y).tickSize(-width, 0, 0).tickFormat(""); // axes

    svg.append("g").classed("x axis", true).attr("transform", "translate(" + 0 + "," + (height - 1) + ")").call(xAxis).selectAll("text").style("text-anchor", "middle").style("font-size", 10);
    svg.append("g").classed("y axis", true).attr("transform", "translate(" + 0 + "," + 0 + ")").call(yAxis);
    svg.append("g").call(yGridlines).classed("gridline", true).attr("transform", "translate(0,0)");
    svg.datum(data);
    ...
    svg.append("path").attr("class", "area above").attr("clip-path", "url(#clip-above)").attr("d", area.y0(function (d) {
      return y(d[measures[1].name]["value"]);
    })).attr("transform", "translate(" + 0 + ",0)").attr("fill", fills[1]);
    svg.append("path").attr("class", "area below").attr("clip-path", "url(#clip-below)").attr("d", area).attr("transform", "translate(" + 0 + ",0)").attr("fill", fills[0]);
    svg.append("path").attr("class", "line1").attr("d", line1).attr("transform", "translate(" + 0 + ",0)").attr("fill", "none").attr("stroke", colors[0]).attr("stroke-width", 3);
    svg.append("path").attr("class", "line2").attr("d", line2).attr("transform", "translate(" + 0 + ",0)").attr("fill", "none").attr("stroke", colors[1]).attr("stroke-width", 3);
    var focus = svg.append("g") // **********
    .style("display", "none"); // create a tooltip

    var tooltip = d3__WEBPACK_IMPORTED_MODULE_0__[/* select */ "C"](".tooltip").attr("opacity", 0).style("display", "none").style("background-color", "#262d33").style("border-radius", "2px").style("padding", "5px").style("position", "absolute").style("pointer-events", "none").html("<div id='value-body'></div>"); // // append the circle at the intersection               // **********
    ...
    function mousemove() {
      var x0 = x.invert(d3__WEBPACK_IMPORTED_MODULE_0__[/* mouse */ "t"](this)[0]),
          // **********
      ...
      var bodyFluff = '<p class="value-headers">Month: <span class="value-headers-body">' + d0[dimension.name]["value"] + '</span></p>' + '<p class="value-headers">Requests | Quote-to-DAT: <span class="value-headers-body">' + Math.round(d[measures[0].name]["value"] * 100) + '%</span></p>' + '<p class="value-headers">Won | Quote-to-DAT: <span class="value-headers-body">' + Math.round(d[measures[1].name]["value"] * 100) + '%</span></p>' + '<p class="value-headers">Difference: <span class="value-headers-body">' + Math.round((d[measures[0].name]["value"] - d[measures[1].name]["value"]) * 100) + '%</span></p>';
      tooltip.html(bodyFluff);

      if (x(parseTime(d[dimension.name]["value"])) < width - 180) {
        tooltip.style("left", d3__WEBPACK_IMPORTED_MODULE_0__[/* mouse */ "t"](this)[0] + 80 + "px").style("top", d3__WEBPACK_IMPORTED_MODULE_0__[/* mouse */ "t"](this)[1] - 25 + "px");
      } else {
        tooltip.style("left", d3__WEBPACK_IMPORTED_MODULE_0__[/* mouse */ "t"](this)[0] - 150 + "px").style("top", d3__WEBPACK_IMPORTED_MODULE_0__[/* mouse */ "t"](this)[1] - 25 + "px");
      }
    ...
    var legend = svg.selectAll(".legend").data(measures).enter().append("g").attr("class", "legend").attr("transform", function (d, i) {
      return "translate(" + (i * 180 - width / 1.2) + "," + (height + margin.bottom / 2) + ")";
    });
    legend.append("circle").attr("cx", width - 24).attr("cy", 9).attr("r", 7).style("fill", function (d, i) {
      return colors[i];
    });
    legend.append("text").attr("x", width - 12).attr("y", 9).attr("dy", ".35em").style("text-anchor", "beginning").style("font-size", 10).text(function (d, i) {
      if (i == 0) {
