(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["splitViolin"] = factory();
	else
		root["splitViolin"] = factory();
})(window, function() {
...
looker.plugins.visualizations.add({
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    try {
      ...
      var group = svg.append('g').attr('transform', "translate(".concat(margin.left, ", ").concat(margin.top, ")")).attr('width', "100%").attr('height', height + "px").classed("group", true); // require 2 pivots, 1 dimension, 1 measure
      ...
      console.log("pivots", pivots); // determine how to format the time pivot (if there is one)
      ...
      pivotDate.forEach(function (element) {
        parseTime.push(d3__WEBPACK_IMPORTED_MODULE_0__[/* timeParse */ "I"](dateDict[element]));
      });
      ...
      data.forEach(function (d) {
        var keys = Object.keys(d[measure.name]); // console.log("keys", keys)
      ...
      console.log("data_ready", data_ready); // DATA ANALYSIS
      ...
      var xNum = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleLinear */ "x"]().domain([-maxDepth, maxDepth]).range([0, xScale.bandwidth()]); // -------------------------------------------------------
      // DRAW PERIPHERALS
      // X axis
      ...
      }).attr("class", "violin").attr("clip-path", "url(#plot-area)");
      var leftViolins = violins.append("path").datum(function (d) {
        return d.values[0].value;
      }).attr("class", "left-violin").style("stroke", "none").style("fill", config.left_color).attr("d", d3__WEBPACK_IMPORTED_MODULE_0__[/* area */ "a"]().x0(function (d) {
        return xNum(-d.length);
      }).x1(function (d) {
        return xNum(0);
      }).y(function (d) {
        return yScale(d.x0);
      }).curve(d3__WEBPACK_IMPORTED_MODULE_0__[/* curveMonotoneY */ "h"]));
      var rightViolins = violins.append("path").datum(function (d) {
        return d.values[1].value;
      }).attr("class", "right-violin").style("stroke", "none").style("fill", config.right_color).attr("d", d3__WEBPACK_IMPORTED_MODULE_0__[/* area */ "a"]().x0(function (d) {
        return xNum(0);
      }).x1(function (d) {
        return xNum(d.length);
      }).y(function (d) {
      ...
      }).attr("width", xScale.bandwidth() / 3).attr("height", 1.75).attr("stroke", function (d) {
