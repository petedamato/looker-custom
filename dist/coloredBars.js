(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["coloredBars"] = factory();
	else
		root["coloredBars"] = factory();
})(window, function() {
...
looker.plugins.visualizations.add({
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    try {
      ...
      var dimensionCreation = function dimensionCreation() {
        var margin = {
          top: 50,
          right: 10,
          bottom: 45,
        ...
        var group = svg.append('g').attr("transform", "translate(".concat(margin.left, ", ").concat(margin.top, ")")).attr("width", "100%").attr("height", height - margin.top - margin.bottom + "px").classed("group", true);
      ...
      var valueAccessor = function valueAccessor(d) {
        return +String(d["value"]).replace(/,/g, '');
      ...
      console.log(data_ready); // data_ready = data_ready.sort(function(a,b) {
      ...
      var _dimensionCreation = dimensionCreation(),
          _dimensionCreation2 = _slicedToArray(_dimensionCreation, 5),
          margin = _dimensionCreation2[0],
          width = _dimensionCreation2[1],
          height = _dimensionCreation2[2],
          svg = _dimensionCreation2[3],
          group = _dimensionCreation2[4]; // scales


      var xScale = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleBand */ "w"]().domain(data_ready.map(function (d) {
        return groupAccessor(d);
      })).range([0, width]).padding(0.2);
      var yScale = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleLinear */ "x"]().domain([0, d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"](data_ready, function (d) {
        return valueAccessor(d);
      })]).range([height, 0]); // draw data

      var bars = group.selectAll("rect").data(data_ready).enter().append("rect").attr("x", function (d) {
        return xScale(groupAccessor(d));
      }).attr("y", function (d) {
        return yScale(valueAccessor(d));
      }).attr("width", xScale.bandwidth()).attr("height", function (d) {
        return height - yScale(valueAccessor(d));
      });

      if (config.bar_display == "no") {
        bars.attr("fill", config.color);
      } else {
        bars.attr("fill", function (d, i) {
          if (i == 0) {
            return "lightgrey";
          } else {
            var priorVal = valueAccessor(data_ready[i - 1]);
            var currVal = valueAccessor(d);
      ...
      var xAxisGenerator = d3__WEBPACK_IMPORTED_MODULE_0__[/* axisBottom */ "b"]().scale(xScale).tickSize(0).tickPadding([10]).tickFormat(d3__WEBPACK_IMPORTED_MODULE_0__[/* timeFormat */ "H"]("%b %-d"));
      var xAxis = group.append("g").call(xAxisGenerator).style("transform", "translateY(".concat(height, "px)")).attr("stroke-opacity", 0.0).attr("font-size", "1em").attr("font-family", "sans-serif");
      var yAxisGenerator = d3__WEBPACK_IMPORTED_MODULE_0__[/* axisLeft */ "c"]().scale(yScale).ticks(5).tickSize(0);

      if (config.currency_type == "dollar") {
        yAxisGenerator.tickFormat(d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"]("$,"));
      ...
      if (config.label == "x-axis" || config.label == "both-axis") {
        var xAxisLabel = group.append("text").attr("x", width / 2).attr("y", height + margin.bottom - 4).attr("fill", "black").style("font-size", "1.4em").text(function (d) {
          return dimension.label;
        }).style("text-anchor", "middle");
      }

      if (config.label == "y-axis" || config.label == "both-axis") {
        var yAxisLabel = yAxis.append("text").attr("x", -30).attr("y", -20).attr("fill", "black").style("font-size", "1.4em").text(function (d) {
          return measure.label;
        }).style("text-anchor", "start");
      }

      var minVal = d3__WEBPACK_IMPORTED_MODULE_0__[/* min */ "s"](data_ready, function (d) {
        return valueAccessor(d);
      });
      var maxVal = d3__WEBPACK_IMPORTED_MODULE_0__[/* max */ "r"](data_ready, function (d) {
        return valueAccessor(d);
      });
      var labelVals = [minVal, maxVal];
      var barText = group.append("g").selectAll("text").data(data_ready).enter().append("text").attr("x", function (d) {
        return xScale(groupAccessor(d)) + xScale.bandwidth() / 2;
      }).attr("y", function (d) {
        return yScale(valueAccessor(d)) - 6;
      }).text(function (d) {
        if (labelVals.includes(valueAccessor(d))) {
          if (config.currency_type == "dollar") {
            return d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"]("$,.0f")(valueAccessor(d));
          } else {
            return d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"](",.0f")(valueAccessor(d));
          }
        } else {
      ...
      }).style("text-anchor", "middle").attr("fill", "#323232").attr("font-size", "0.75em").attr("font-family", "sans-serif");
    } catch (error) {
      if (environment == "prod") {
