  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    try {
      ...
      var drawTooltip = function drawTooltip() {
        ...
        var ttdate = x.invert(d3__WEBPACK_IMPORTED_MODULE_0__[/* mouse */ "t"](tooltipBox.node())[0]);
        var ttdayofwk = d3__WEBPACK_IMPORTED_MODULE_0__[/* timeFormat */ "H"]("%u")(ttdate);

        var _startAndEndWeek = startAndEndWeek(ttdate),
            _startAndEndWeek2 = _slicedToArray(_startAndEndWeek, 2),
            mon = _startAndEndWeek2[0],
            nextMon = _startAndEndWeek2[1]; // calculate the closest monday date to the mouse date since data is weekly


        var closestMonday = function closestMonday(ttdayofwk) {
        ...
        var wkdt = closestMonday(ttdayofwk).setHours(0, 0, 0, 0); // reformat data so that can easily filter on it for the tooltip data
        ...
        tooltipData.forEach(function (d) {
          var filtered = d.info.filter(function (i) {
        ...
        ttdata.sort(function (a, b) {
          return b.info[0].value - a.info[0].value; // does do something
        }); // add elements/shapes

        tooltipLine.attr("stroke", "#a6a6a6").attr("x1", x(wkdt)).attr("x2", x(wkdt)).attr("y1", 0).attr("y2", height).attr("stroke-dasharray", "3, 3");
        console.log('TTDATA', ttdata);
        console.log("x", x(wkdt), wkdt);
        console.log(width, height);
        tooltipCircles.selectAll("circle").remove();
        tooltipCircles.selectAll("circle").data(ttdata).enter().append("circle").attr("r", 3).attr("cx", function (d) {
          return x(d.info[0].date);
        }).attr("cy", function (d) {
          return y(d.info[0].value);
        }).style('fill', '#3a4245'); // have title case for the labels in the tooltip text
        ...
        tooltip.html(d3__WEBPACK_IMPORTED_MODULE_0__[/* timeFormat */ "H"]("%b %-d, %Y")(wkdt) + "<hr>").style("display", "block").style("border", "solid").style("border-color", "lightgrey").style("border-width", ".5px").style("font-size", 12) // .style("left", d3.event.pageX + 15 + "px")
        .style("top", d3__WEBPACK_IMPORTED_MODULE_0__[/* event */ "j"].pageY - 10 + "px").selectAll().data(ttdata).enter().append("div").style("color", "#323232").style("font-size", 12) // .html(d => titleCase(d.name) + ':&nbsp' + '<span style="float:right;">' + d3.format(",.0f")(d.info[0].value) + '</span>')
        .html(function (d) {
          if (config.currency_type == "dollar") {
            return titleCase(d.name) + ':&nbsp' + '<span style="float:right;">' + d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"]("$,.0f")(d.info[0].value) + '</span>';
          } else {
            return titleCase(d.name) + ':&nbsp' + '<span style="float:right;">' + d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"](",.0f")(d.info[0].value) + '</span>';
          }
        }); // if mouse is past 85% of x width, move the tooltip text box to the left of the line

        if (d3__WEBPACK_IMPORTED_MODULE_0__[/* event */ "j"].pageX > width * .85) {
          tooltip.style("left", d3__WEBPACK_IMPORTED_MODULE_0__[/* event */ "j"].pageX - 110 + "px");
        } else {
          tooltip.style("left", d3__WEBPACK_IMPORTED_MODULE_0__[/* event */ "j"].pageX + 15 + "px");
        }
      }; // -----------------------------------------------------------------------------


      // console.log(config)
      console.log("ELEMENT", element); // first we set the dimensions and margins of the graphic
      ...
      var svg = d3__WEBPACK_IMPORTED_MODULE_0__[/* select */ "C"](element).select('svg').html('').attr('width', '100%').attr('height', '100%');
      var group = svg.append('g').attr("transform", "translate(".concat(margin.left, ", ").concat(margin.top, ")")).attr("width", "100%").attr("height", height + "px").classed("group", true); // determine which parseTime to use by looking at the data columns

      var parseTimeDay = d3__WEBPACK_IMPORTED_MODULE_0__[/* timeParse */ "I"]("%Y-%m-%d");
      var parseTimeMonth = d3__WEBPACK_IMPORTED_MODULE_0__[/* timeParse */ "I"]("%Y-%m");
      var parseTime;
      var dimensions_ordered = queryResponse.fields.dimension_like;
      var measures_ordered = queryResponse.fields.measure_like; // determine if parseTime should be by day or by month
      ...
      console.log("DATA", data); // divide the data into 3 separate groups

      var colors = ["#27566b", "#007b82", "#8cbb61"];
      var data1 = [];
      var data2 = [];
      var data3 = [];
      ...
      console.log("data1, data2, data3", data1, data2, data3); // parseTime becomes a function using the d3.timeParse() function, it's to make the dates
      // in our csv file readable to our machine

      var x = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleTime */ "B"]().domain(d3__WEBPACK_IMPORTED_MODULE_0__[/* extent */ "k"](data, function (d) {
        return d[dimensions_ordered[0].name].value;
      })).range([0, width]);
      var minVal = Math.min(Math.min.apply(Math, _toConsumableArray(data1.map(function (d) {
        return d.value;
      }))), Math.min.apply(Math, _toConsumableArray(data2.map(function (d) {
        return d.value;
      }))), Math.min.apply(Math, _toConsumableArray(data3.map(function (d) {
        return d.value;
      }))));
      var maxVal = Math.max(Math.max.apply(Math, _toConsumableArray(data1.map(function (d) {
        return d.value;
      }))), Math.max.apply(Math, _toConsumableArray(data2.map(function (d) {
        return d.value;
      }))), Math.max.apply(Math, _toConsumableArray(data3.map(function (d) {
        return d.value;
      }))));
      var y = d3__WEBPACK_IMPORTED_MODULE_0__[/* scaleLinear */ "x"]().domain([minVal, maxVal]).range([height, 0]); // -----------------------------
      ...
      data3 = data3.filter(function (d) {
        return d.date < dateCutoff;
      }); // actuals
      // data2 = data2.filter(d => d.date >= d3.max(data3, d => d.date)) // forecast

      console.log("cutoff data2, data3", data2, data3); // ------------------------------

      var line = d3__WEBPACK_IMPORTED_MODULE_0__[/* line */ "q"]().defined(function (d) {
        return d.value != null && d.value != 0 && x(d.date) >= 0;
      }).curve(d3__WEBPACK_IMPORTED_MODULE_0__[/* curveNatural */ "i"]).x(function (d) {
        return x(d.date);
      }).y(function (d) {
        return y(d.value);
      });
      var movingLine = d3__WEBPACK_IMPORTED_MODULE_0__[/* line */ "q"]().defined(function (d) {
        return d.value != null && x(d.date) >= 0;
      }).curve(d3__WEBPACK_IMPORTED_MODULE_0__[/* curveNatural */ "i"]).x(function (d) {
        return x(d.date);
      }).y(function (d) {
        return y(d.value);
      });
      var POINTS_AVERAGE = 7;
      var movingData = [];
      data3.forEach(function (d, index) {
        ...
        for (var j = 0; j <= POINTS_AVERAGE; j++) {
          if (index - j >= 0) {
      ...
      x.domain(d3__WEBPACK_IMPORTED_MODULE_0__[/* extent */ "k"](data1, function (d) {
        return d.date;
      }));
      y.domain(d3__WEBPACK_IMPORTED_MODULE_0__[/* extent */ "k"](data1, function (d) {
        return d.value;
      }));
      var rawLine = group.append("path").data([data3]).attr("class", "only-line").attr("d", line);
      var budgetLine = group.append("path").data([data1]).attr("class", "budget-line").attr("d", line);
      var forecastLine = group.append("path").data([data2]).attr("class", "forecast-line").attr("d", line);

      if (config.moving_average == "yes") {
        group.append("path").data([movingData]).attr("class", "moving").attr("d", movingLine);
        rawLine.attr("class", "line");
      } else {
        var newRawLine = group.append("path").data([data3]).attr("class", "only-line").attr("d", line);
      } // console.log("drew lines")


      var legend = group.selectAll(".legend").data(colors).enter().append("g").attr("class", "legend").attr("transform", function (d, i) {
        return "translate(" + (-100 + i * 110 - width / 2) + "," + (height + margin.bottom / 2.3) + ")";
      });
      legend.append("rect").attr("x", width - 12).attr("y", 9).attr("height", 8).attr("width", 19).style("fill", function (d, i) {
        return colors[i];
      ...
      legend.append("rect").attr("x", width - 12).attr("y", 9).attr("height", 8).attr("width", 5).style("fill", function (d, i) {
        return colors[i];
      }).attr("opacity", function (d, i) {
        if (i == 0) {
          return 0;
      ...
      legend.append("rect").attr("x", width - 5).attr("y", 9).attr("height", 8).attr("width", 5).style("fill", function (d, i) {
        return colors[i];
      }).attr("opacity", function (d, i) {
        if (i == 0) {
          return 0;
      ...
      legend.append("rect").attr("x", width + 2).attr("y", 9).attr("height", 8).attr("width", 5).style("fill", function (d, i) {
        return colors[i];
      }).attr("opacity", function (d, i) {
        if (i == 0) {
          return 0;
      ...
      legend.append("text").attr("x", width - 22).attr("y", 13).attr("dy", ".35em").style("text-anchor", "end").style("font-size", 10).text(function (d, i) {
        var arr = ["Actual", "Target", "Forecast"];
        return arr[i];
      }); // console.log("drew legend")
      // Add the x Axis

      group.append("g").attr("transform", "translate(0," + height + ")").call(d3__WEBPACK_IMPORTED_MODULE_0__[/* axisBottom */ "b"](x).ticks(4)); // Add the y Axis

      if (config.currency_type == "dollar") {
        group.append("g").call(d3__WEBPACK_IMPORTED_MODULE_0__[/* axisLeft */ "c"](y).ticks(5).tickFormat(function (d) {
          if (d >= 1000000) {
            return d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"]("$,")(d / 1000000) + "M";
          } else {
            return d3__WEBPACK_IMPORTED_MODULE_0__[/* format */ "l"]("$,")(d);
          }
        }));
      } else {
        group.append("g").call(d3__WEBPACK_IMPORTED_MODULE_0__[/* axisLeft */ "c"](y).ticks(5).tickFormat(function (d) {
      ...
      console.log("tooltip-div", d3__WEBPACK_IMPORTED_MODULE_0__[/* select */ "C"]("#tooltip-div"));
      var tooltip = d3__WEBPACK_IMPORTED_MODULE_0__[/* select */ "C"]("#tooltip-div").style("position", "absolute").style("padding", "5px").style("background-color", "#ffffff") // .style("border", "solid")
      // .style("border-color", "lightgrey")
      // .style("border-width", ".5px")
      .attr("class", "tooltip-textbox"); // group element to house all the tooltip things in svg

      var tt = group.append("g").classed("tooltip", true);
      var tooltipLine = tt.append("line");
      var tooltipCircles = tt.append("g"); // box that covers entire plot area and is see-through

      var tooltipBox = tt.append("rect").attr("class", "tooltip-area-rect").attr("width", width).attr("height", height).attr("opacity", 0).on("mousemove", drawTooltip).on("mouseout", removeTooltip); // calculate dates

      var startAndEndWeek = function startAndEndWeek(date) {
        var dt = new Date(date);
        var monday = new Date(dt);
        monday.setDate(monday.getDate() - monday.getDay() + 1);
        var nextMonday = new Date(dt);
