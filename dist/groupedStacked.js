(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["groupedStacked"] = factory();
	else
		root["groupedStacked"] = factory();
})(window, function() {
...
looker.plugins.visualizations.add({
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    this.clearErrors();
    var svg = d3__WEBPACK_IMPORTED_MODULE_0__[/* select */ "C"](element).select('svg').html('').attr('width', '100%').attr('height', '100%'); // const parseDate = d3.timeParse("%Y-%m-%d")
    ...
    bars.enter().append("text").attr("class", "item-text").attr("x", function (d, i) {
    ...
    groupA.append("text").attr("transform", "translate(" + -24 + ",-29)").attr("font-weight", "bold").text(function () {
      var text = Object.keys(dat_data[0]).filter(function (d) {
    ...
    var legend = groupA.selectAll(".legend").data(groups).enter().append("g").attr("class", "legend").attr("transform", function (d, i) {
      return "translate(" + (-40 + i * 110 - width / 2.05) + "," + -margin.top * .9 + ")";
    });
    ...
    if (queryResponse.fields.dimensions.length == 0) {
      ...
      this.addError({
        title: "Pivot Chart",
        message: "This chart requires one pivot"
      });
      return;
