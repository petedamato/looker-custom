export const object = {
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    const margin = {top: 90, right: 20, bottom: 20, left:56};
    ...
    pivotKeys.forEach((entry)=>{
      if (entry.includes("null")) {
        ...
        listPeg.append($('<option></option>').attr('value', entry).text("All").attr("selected","selected"));
      } else {
        listPeg.append($('<option></option>').attr('value', entry).text(entry));
      }
    })
    ...
    let dataColumns = ["asp_quotes","price_requests.quoted_shipment_count","price_requests.available_shipment_count"].reverse()
    ...
    svg.append('rect').attr('width', (width + margin.left/2 + margin.right/2))
        ...
        .attr('transform', 'translate(' + 20 + ',' + (heightA + 16) + ')')
        .classed("backdrop", true).style("fill", "#e9eaf3")

    const groupB = svg.append('g').attr('width', "100%").attr('height', ((heightB - margin.top - margin.bottom) + "px")).attr('transform', 'translate(' + margin.left + ',' + (heightA + margin.top/2) + ')').classed("groupB", true).style("background-color", "black")
    ...
    const color = d3.scaleOrdinal()
        .domain(dataColumns)
        .range(['#27566b','#8cbb61','#007b82'])

    const marginB = {top: 35, right: 20, bottom: 20, left: 40},
      y2 = d3.scaleLinear().range([heightB, 0]).domain([0,.1])
    ...
    const groupABarsRects = groupABars
          .selectAll("rect")
    ...
    groupABars.exit().remove()

     groupABarsRects
          .exit().remove()


    const groupAText = groupA
      ...
      var legend = groupA.selectAll(".legend")
        .data(dataColumns)
          .enter()
            .append("g")
              .attr("class", "legend")
              .attr("transform", function(d, i) { return "translate(" + (margin.left/2 + (i * width/3)) + "," + (-margin.top/2.7) + ")"; });
        ...
        legend.append("text")
            ...
            .style("font-size",10)
            .text((d,i)=>{
              const dat = ["Quoted shipments, ASP", "Quoted shipments, other", "Available shipments, not quoted"].reverse()
              return dat[i]
            });
    ...
    const path = groupB.append("path")
      ...
      .attr("d", d3.line()
        ...
        .y(function(d) { return y2(d["price_requests.win_percent"]["$$$_row_total_$$$"].value) }))
      .transition()
      .duration(750)
        .attr("d", d3.line()
        .x(function(d) { 
