export const object = {
  ...
  const x = d3.scaleLinear()
    ...
    const pointsB = [{
        ...
        x: x.range()[1] - ((width - x(data[0][measures[1].name]["value"])*0.1)/2), y: y(measures[0].name) - 1
      },{
        x: x.range()[0] + ((width - x(data[0][measures[1].name]["value"])*0.1)/2), y: y(measures[0].name) - 1
      }];
