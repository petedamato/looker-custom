...
looker.plugins.visualizations.add({
    ...
    updateAsync: function(data, element, config, queryResponse, details, done, environment = "prod") {
        ...
        try {
            ...
            // march -> 3 -> 1 + 1 -> 2, but should be 1
            // june & sept are also 1 quarter ahead
            // dec -> 12 -> 4 + 1 -> 5 - 4 -> 1, but should be 4 
            function getQuarter(d) {
                d = d || new Date();
                var m = Math.floor(d.getMonth()/3) + 1;
                const test = m > 4? m - 4: m;
                console.log("quarter", d, m, test)
                return m > 4? m - 4 : m;
                }
            ...
            data_remix.sort(function(a,b) {
                return parseTime(a[dimension.name].value) - parseTime(b[dimension.name].value)
                    ...
                    for (let m = 0; m <= (getMonth(new Date())); m++) {
