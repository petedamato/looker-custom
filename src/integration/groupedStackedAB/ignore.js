...
looker.plugins.visualizations.add({
    ...
    element.innerHTML = `
      <style>
        ...
        #testB {
          width: 100%;
          height: 200px;
          background-color: rgba(39, 86, 107,.1);
        }
        #test .item {
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    console.log("Test2")
      ...
      // Error handling breaks whole chart if input data goes out of range specified below
      if (!handleErrors(this, queryResponse, {
        min_pivots: 0, max_pivots: 4,
    ...
    const dimension = dataLoad.queryResponse.fields.dimension_like[0]
const measures = dataLoad.queryResponse.fields.measure_like
const pivots = dataLoad.queryResponse.fields.pivots
const dataColumns = Object.keys(dataLoad.data[0]).splice(1)
const pivotedColumns = Object.keys(dataLoad.data[0][dataColumns[0]])
const dat_data = dataLoad.data;
...
const formatDate = d3.timeFormat("%b %d, '%y")
    ...
    // const width = element.clientWidth - margin.left - margin.right
    // const height = element.clientHeight - margin.top - margin.bottom
    const width = +chart.style('width').replace('px', '') - margin.left - margin.right
    const height = +chart.style('height').replace('px', '')/ 2

    const heightA = height * (2/3)

    const groupA = svg.append('g')
      .attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('width', "100%")
      .attr('height', "400px").classed("testA", true)

    const groupB = svg.append('g').attr('transform', 'translate(' + margin.left + ',' + (heightA + margin.top + margin.top) + ')').classed("testB", true)


    const data = manip_data
